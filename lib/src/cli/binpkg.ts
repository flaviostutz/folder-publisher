 ...
 * extracts all files. The data package's own npmdata config in package.json is never
...
export async function binpkg(binDir: string, args: string[]): Promise<void> {
  ...
  // No configCwd is passed, so the data package's own npmdata config is not loaded.
  const exitCode = await cli(['node', 'npmdata', ...args, '--packages', pkgName], process.cwd());
  // eslint-disable-next-line unicorn/no-process-exit
